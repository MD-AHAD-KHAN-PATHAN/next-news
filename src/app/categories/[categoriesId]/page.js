...
const DynamicNewsPage = ({ params, searchParams }) => {
    ...
    return (
        <Box className='my-10'>
            <h1 className='mb-4 border-b-2 text-2xl font-bold pb-4'><span className='border-b-4 border-red-500 pb-4'>{searchParams.category.toUpperCase()}</span></h1>

            <Grid container rowSpacing={3} columnSpacing={3} className="mt-2">
                {
                    data?.map(category => <Grid key={category._id} item xs={6}>
                        <Link href={`/${category.category.toLowerCase()}/${category._id}`}>
                            <Card>
                                <CardActionArea>
                                    <CardMedia sx={{
                                        "& img": {
